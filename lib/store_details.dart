...
class _StoreDetailsPageState extends State<StoreDetailsPage> {
  ...
  Future<void> fetchStoreDetails() async {
    try {
      final response = await http.get(
        Uri.parse('${ApiService.baseUrl}/stores?id=${widget.storeId}'),
      );
  ...
  Widget _buildImageSlider() {
    ...
    if (imagesField is List && imagesField.isNotEmpty) {
      images = imagesField
          .map((img) {
            if (img is Map)
              return (img['image_url'] ?? img['imageUrl'] ?? '').toString();
            return img.toString();
          })
    ...
    if (images.isEmpty) {
      final singleUrl =
          storeDetails!['image_url'] ?? storeDetails!['imageUrl'] ?? '';
  ...
  Widget _buildDetailRow(
      IconData icon, String value, String? url, String? phone) {
    return Row(
      children: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          ModernAppBar(title: storeDetails?['store_name'] ?? 'Store Details'),
                          ...
                          child: Row(
                            ...
                            children: [
                              _buildSocialIcon('assets/facebook.png',
                                  storeDetails!['facebook_url']),
                              _buildSocialIcon('assets/instagram.png',
                                  storeDetails!['instagram_url']),
                              _buildSocialIcon('assets/whatsapp.png',
                                  storeDetails!['whatsapp_number']),
                              _buildSocialIcon('assets/tiktok.png',
                                  storeDetails!['tiktok_url']),
                            ],
                          ),
                        ),
                        const SizedBox(height: 20),
